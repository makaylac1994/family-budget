...
function AmountEditCell({ value, type, onCommit }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <span className="font-semibold text-sm" style={{ color }}>{type === 'income' ? '+' : '-'}$</span>
      <input
        type="number"
        min="0"
        step="0.01"
                  ...
                  <React.Fragment key={t.id}>
                    <tr className="border-b last:border-0" style={{ borderColor: COLORS.border }}>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        {t.splits && t.splits.length ? (
                          <span className="font-semibold text-sm" style={{ color: t.type === 'income' ? COLORS.teal : COLORS.coral }}>
                            {t.type === 'income' ? '+' : '-'}{formatCurrency(t.amount)}
                          </span>
                        ) : (
                          <AmountEditCell value={t.amount} type={t.type} onCommit={(amt) => updateAmount(t.id, amt)} />
                        )}
                      </td>
                      <td className="px-4 py-2.5 text-right">
